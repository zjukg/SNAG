...
def set_optim(opt, model_list, freeze_part=[], no_freeze_part=[], accumulation_step=None):
    named_parameters = []
    param_name = []
    for model in model_list:
        ...
        for n, p in model_para:
            if not any(nd in n for nd in freeze_part):
                if len(no_freeze_part) == 0 or any(nd in n for nd in no_freeze_part):
    ...
    if opt.model_name == "SNAG":
        no_decay = ['weight_raw', 'bias', 'multi_loss_layer']
        large_lr = ['multi_loss_layer']

        parameters = [
            {'params': [p for n, p in named_parameters if not any(nd in n for nd in no_decay)], "lr": opt.lr, 'weight_decay': opt.weight_decay},
            {'params': [p for n, p in named_parameters if any(nd in n for nd in no_decay) and not any(nd in n for nd in large_lr)], "lr": opt.lr, 'weight_decay': 0.0},
            {'params': [p for n, p in named_parameters if any(nd in n for nd in no_decay) and any(nd in n for nd in large_lr)], "lr": 5 * opt.lr, 'weight_decay': 0.0}
        ]
    else:
        parameters = [
